...
## System design benchmarks & empirical performance measurements

All system design claims are backed by actual empirical measurements generated by standard benchmarking tools against a default dataset of **10,000 products, 100 users, 500 conversations, and 20,000 messages** (configurable via CLI flags). View [BENCHMARKS.md](BENCHMARKS.md) for full metrics, latency percentiles, executionStats, and auto-generated resume bullet points.

### Benchmarking Tools & Metrics Measured

1. **MongoDB `.explain("executionStats")`**:
   - **Metrics**: `executionTimeMillis`, `totalDocsExamined`, `totalKeysExamined`, `winningPlan.stage`.
   - **Method**: Safely compares indexed queries (`IXSCAN`) against unindexed collection scans (`COLLSCAN`) using `.hint({ $natural: 1 })` without dropping indexes.
2. **Autocannon**:
   - **Metrics**: `requests/sec`, `average latency`, `p50`, `p95`, `p99`, `throughput (MB/sec)`, `concurrency`.
   - **Method**: Evaluates REST API endpoints across **10, 25, 50, and 100 concurrent users**.
3. **Artillery / Socket.io Load Testing**:
   - **Metrics**: `concurrent users`, `delivery latency (avg, p50, p95, p99)`, `dropped messages`, `reconnects`, `delivery success rate (%)`, and `WebSocket vs polling latency overhead`.
   - **Method**: Simulates real-time chat traffic across **1, 10, 50, and 100 concurrent WebSocket clients**.
4. **Connection Pooling Benchmark**:
   - **Metrics**: Throughput (req/sec) and latency distributions under 50 concurrent DB read queries.
   - **Method**: Empirical comparison between `maxPoolSize: 1` and `maxPoolSize: 10`.
5. **3-Stage Read Caching**:
   - **Metrics**: Latency and `X-Cache` response headers across Cold Cache -> Warm Cache -> Post-Write Invalidation.
6. **Automated Node.js Integration Tests**:
   - Functional API verification validating JWT authentication (200/401), CRUD operations, ownership authorization (403), and rate limit enforcement (429).

### Benchmark Credibility & Auditability

Raw output from every benchmarking tool is stored as structured JSON for complete auditability:

```
backend/scripts/bench/results/
├── explain.json           # Raw MongoDB explain stats (IXSCAN vs COLLSCAN)
├── autocannon.json        # Raw REST load test stats (10, 25, 50, 100 concurrency)
├── pooling.json           # Connection pool size benchmark (Pool Size 1 vs 10)
├── cache.json             # Cold -> Warm -> Post-invalidation cache results
├── artillery.json         # Socket.io chat scaling results (1-100 users)
├── hardware.json          # Detected CPU, RAM, OS, Node & MongoDB versions
└── benchmark-results.json # Master aggregated dataset used to build BENCHMARKS.md
```

### Reproducing Exact Results

To reproduce all benchmarks and automatically regenerate [BENCHMARKS.md](BENCHMARKS.md):

```bash
# 1. Install dependencies
npm install

# 2. Seed default benchmark dataset (10,000 products, 100 users, 500 conversations, 20,000 messages)
npm run seed -- --bench

# 3. Run full benchmark suite & update BENCHMARKS.md
npm run benchmark
```
